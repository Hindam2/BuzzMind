...
function activateQuestion(id) {
  ...
  block.innerHTML = `
    <div class="q-header">
      ...
      <div class="q-actions">
        <button class="q-btn" onclick="deleteQuestion('${id}')" title="Delete">🗑</button>
...
function collectQuizPayload() {
  ...
  document.querySelectorAll('.question-block:not(.empty-block)').forEach((block) => {
    ...
    questions.push({ text, imageUrl: null, answers, correctIndex });
  });
...
async function launchQuiz() {
  ...
  if (!payload.title) {
    showToast('⚠️ Please enter a quiz title before launching!', 'error');
    document.getElementById('quizTitle').focus();
    return;
  }
  if (payload.questions.length === 0) {
    showToast('⚠️ Please write at least one question!', 'error');
    return;
  }
  if (payload.questions.some((q) => q.answers.some((a) => !a))) {
    showToast('⚠️ Please fill in all answer choices!', 'error');
    return;
  }
